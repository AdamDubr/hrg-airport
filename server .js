...
async function getFlights(direction) {
  ...
  const now = new Date();
  const from = fr24date(new Date(now.getTime() - 8 * 60 * 60 * 1000));
  const to   = fr24date(new Date(now.getTime() + 8 * 60 * 60 * 1000));
  const url = `https://fr24api.flightradar24.com/api/flight-summary/light?airports=${airport}&flight_datetime_from=${from}&flight_datetime_to=${to}&limit=50&sort=asc`;
  const data = await fr24fetch(url);
  return data.data || [];
}

app.get('/api/arrivals', async (req, res) => {
  try { res.json({ data: await getFlights('inbound') }); }
  catch (err) { res.status(500).json({ error: err.message }); }
});

app.get('/api/departures', async (req, res) => {
  try { res.json({ data: await getFlights('outbound') }); }
  catch (err) { res.status(500).json({ error: err.message }); }
});
